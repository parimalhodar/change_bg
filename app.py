...
if images:
        ...
        try:
            with Image.open(image) as img:
                ...
                with col2:
                    ...
                    base_name = image.name.split('.')[0] if image.name else "processed"
                    if bg_option == "Remove Only (Default)":
    ...
    # Show example of what each option does
    st.markdown("### How it works:")
    st.markdown("""
    1. **Remove Only (Default)**: Removes background, outputs PNG with transparency
    2. **Solid Color**: Replaces background with your chosen color
    3. **Preset Backgrounds**: Uses backgrounds from the backgrounds folder
    4. **Custom Background**: Upload your own background image
    """)